...
The browser bookmarklet is generated by `app/lib/bookmarklet.ts`, installed by
`public/bookmarklet.js`, and lands on `app/routes/add.tsx`. Keep the target derived from the current
deployment origin so forks require no code change. The bookmarklet should carry only `location.href`;
do not put identity or family data in its URL. Preserve editable metadata fallbacks and the guarded,
